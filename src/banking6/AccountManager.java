...
public class AccountManager {
	...
	// 자동저장
	public void autoSaveOption(AutoSaver auto) {
		
		// 저장옵션 선택시 자동저장선택메뉴 출력
		...
		if(keyInput==1) {	// 자동저장 실행 -> start()
			// isAlive() : 해당 스레드의 run()메서드가 종료됐는지에 따라 살아있는지 죽었는지 확인할때 사용
			if(!auto.isAlive()) {	// 자동저장이 실행중이지 않으면
				// 데몬스레드로 실행하려면 반드시 start() 메서드 실행 전에 setDaemon(true)로 설정
				auto.setDaemon(true);
				auto.start(); // 스레드 작업 시작
		...
		else if(keyInput==2) {	// 자동저장 정지 -> interrupt()
			if(auto.isAlive()) { // 자동저장이 실행중이면
				// 일시정지 상태에서 InterruptException 예외를 발생시켜 대기상태나 종료상태로 처리 기능
				auto.interrupt();
